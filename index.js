...
async function start() {
    ...
    if (savedCredsExist()) {
        ...
        try {
            let confirm = await continueConfirmation();
            if (!confirm) {
                doneSetupExit();
            }
        } catch(err) {
...
async function loadCredentialsPrompts() {
    ...
    if (method === 'none') {
        ...
        try {
            ...
            console.log(err);
            console.log(`${chalk.red(`Failed to get ${method === 'pwd' ? 'password' : 'pin'}... exiting...`)}`);
            process.exit();
        }
    } else {
...
async function testCredentials() {
    ...
    try {
        ...
        try {
            let teacherId = await loginCodeHS(page);
            if (!fs.existsSync(path.join(__dirname, 'secrets', 'teacher.json'))) {
                await writeFileAsync(path.join(__dirname, 'secrets', 'teacher.json'), JSON.stringify({teacherID: teacherId}));
            }
            spinner.succeed(`${chalk.bold('Login credentials valid')}`);
        } catch(err) {

            // Need to tell if credentials were wrong vs other error such as internet not connected
            console.log(err);
...
async function loginCodeHS(pg) {
    ...
    if (warningsLength === 0) {
        ...
        throw new Error("There were " + warningsLength + " warnings in the form");
    }
}
            ...
            async (arr_assignmentIDs, obj, TEMPLATE_STUDENT_URL, date_dueDate, arr_obj_students, downloadCode, obj_studentEmail) => {
                ...
                await Promise.all(arr_obj_students.map(async (studentObject) => {
                    await limiter.schedule(() => {
                        const allTasks = arr_assignmentIDs.map(async (key) => {
                            return new Promise((res, rej) => {
                                ...
                                xhr.onerror = async function (err) {
                                    console.error("[aerror] xhr error: " + err.message);
                                };
                        ...
                        return Promise.all(allTasks).catch(err => {
                            console.error("[aerror] error processing all students: " + err.message);
                        });
                    }).catch(err => {
                        console.error("[aerror] Limiter error: " + err.message);
                    });
                ...
                Object.keys(studentObj.assignments).forEach(assignmentIDs => {
                    ...
                    if (!assignmentName.includes('--Problem Removed--')) {
                        ...
                        if (codes.length > 0 && codes[0].code !== null) {
                            if (rmFormatChars) {
                                //console.info('\n[ainfo] removing formatting and writing code\n ' + codes[codes.length - 1].code + "\n");
                                writeQueue.push(writeFileAsync(path.join(folderPath, studentIdentifier + '.js'), removeFormattingCharacters(unescape(codes[codes.length - 1].code))));
                            } else {
                                //console.info('\n[ainfo] writing code\n ' + codes[codes.length - 1].code + "\n");
                                writeQueue.push(writeFileAsync(path.join(folderPath, studentIdentifier + '.js'), unescape(codes[codes.length - 1].code)));
                            }
                        }
                    }
                });
